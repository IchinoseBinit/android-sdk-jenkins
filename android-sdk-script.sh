...
RUN unzip commandlinetools-linux-8092744_latest.zip -d android-sdk/

# Creating a directory and adding a sub folder as the sdk manager would not work without it
RUN sudo mkdir ./android-sdk/latest 
RUN sudo mv ./cmdline-tools/*  ./latest/ 
RUN sudo  mv ./latest ./cmdline-tools/

# Accepting the licenses in sdkmanager
RUN yes | ./sdkmanager --licenses

# Creating an image of the android v
RUN yes | ./sdkmanager 'system-images;android-30;google_apis;x86'

RUN yes | ./sdkmanager 'system-images;android-30;google_apis;x86'

# sudo yes | ./sdkmanager 'platforms;android-31'
# sudo yes | ./sdkmanager 'platforms;android-30'
# sudo yes | ./sdkmanager 'platforms;android-29'
# sudo yes | ./sdkmanager 'platforms;android-28'

# sudo yes | ./sdkmanager "build-tools;30.0.2"
# sudo yes | ./sdkmanager "build-tools;29.0.2"
# sudo yes | ./sdkmanager "build-tools;28.0.2"
